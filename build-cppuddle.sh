...
DOWNLOAD_URL="https://github.com/SC-SGS/CPPuddle.git"
if [[ ! -d ${DIR_SRC} ]]; then
    git clone ${DOWNLOAD_URL} ${DIR_SRC}
fi
...
${CMAKE_COMMAND} -H${DIR_SRC} -B${DIR_BUILD} -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_CXX_COMPILER=$CXX -DCMAKE_BUILD_TYPE=${BUILD_TYPE} -DCMAKE_INSTALL_PREFIX=${DIR_INSTALL}/cppuddle -DCPPUDDLE_WITH_TESTS=OFF -DCPPUDDLE_WITH_COUNTERS=ON -DCPPUDDLE_WITH_MULTIGPU_SUPPORT=OFF  -DHPX_DIR=$INSTALL_ROOT/hpx/${LIB_DIR_NAME}/cmake/HPX/ -DCPPUDDLE_WITH_HPX=OFF -DCPPUDDLE_WITH_HPX_MUTEX=OFF
